...
SWEEP_LOOKBACK = 20      # Look back 20 candles for high/low
SWEEP_THRESHOLD = 0.002  # 0.2% wick threshold
...
def calculate_indicators(df):
    ...
    df['chop'] = df['kc_atr'] / df['close'] * 100 * 10 
...
# ==========================================
# 4. TRIPLE ENGINE LOGIC
# ==========================================

def detect_liquidity_sweep(df):
    """
    Detects BSL (Bear Sweep) and SSL (Bull Sweep).
    BSL: Price wicks above recent high then reverses down.
    SSL: Price wicks below recent low then reverses up.
    """
    if len(df) < SWEEP_LOOKBACK + 1: return None
    
    # Look at previous candles (exclude current)
...
def detect_engines(df):
    ...
    # --- Engine 3: Liquidity Sweep ---
    sweep_type = detect_liquidity_sweep(df)
    
    # --- Combined State ---
    # Priority: Sweep > Mega Sqz > Sqz > Expansion
    state = "NEUTRAL"
    if sweep_type:
        state = sweep_type
    elif sma_sqz_on and bb_sqz_on:
        state = "MEGA SQZ"
    elif sma_sqz_on:
        state = "SMA SQZ"
    elif bb_sqz_on:
    ...
    return state, sma_dist_pct, bb_width, duration, sweep_type
...
def calculate_score(df, btc_regimes, state, sma_dist, bb_width):
    ...
    # Bonus for Sweep (Reversal)
    if "SWEEP" in state:
        score += 30 # High base score for confirmed liquidity events
        details['quality'] = "LIQUIDITY SWEEP"
    elif state == "MEGA SQZ":
        score += 35 
    ...
    # 2. Chop Score
    chop = last['chop']
    if chop < 2.0: score += 20
    elif chop < 3.0: score += 15
    elif chop < 4.0: score += 5

    # 3. BTC Alignment
    local_trend = "BULL" if last['close'] > last['sma_20'] else "BEAR"
    
    # If Sweep is BEAR, we prefer BTC Bearish. If Sweep is BULL, we prefer BTC Bullish.
    if "BEAR" in state: local_trend = "BEAR"
    if "BULL" in state: local_trend = "BULL"
...
def analyze_pair(symbol, timeframe, btc_regimes):
    ...
    state, sma_dist, bb_width, duration, sweep = detect_engines(df)
    score, tier, details = calculate_score(df, btc_regimes, state, sma_dist, bb_width)
    
    last = df.iloc[-1]
    setup_type = None
    
    if "SWEEP" in state: setup_type = "🚨 LIQUIDITY SWEEP"
    elif state == "MEGA SQZ" and duration > 5: setup_type = "💥 MEGA SETUP"
    ...
    return {
        ...
        'rsi': last['rsi'], 'chop': last['chop'], 'vol_ratio': last['vol_ratio'],
...
def run_scan(pairs, timeframes, btc_regimes, results_placeholder, session_placeholder, min_score):
    ...
    # Sort: Sweeps First, then MEGA SQZ, then Score
    ...
    with results_placeholder.container():
        # Filter by Score AND interesting State
        filtered = [r for r in all_results if r['score'] >= min_score and ("SQZ" in r['state'] or "SWEEP" in r['state'] or r['state'] == "EXPANSION")]
        ...
        for idx, res in enumerate(filtered):
            ...
            elif res['score'] >= 50: css_class = "low"

            sym = str(res.get('symbol', 'UNKNOWN'))
            tf = str(res.get('timeframe', '?'))
            price = res.get('price', 0)
            rsi = res.get('rsi', 0)

            html = f"""
            <div class="card {css_class}">
                <h4>{sym} <small>({tf})</small></h4>
                <h3 style="color:white;">{res['tier']}</h3>
                <p><b>State:</b> {res['state']}</p>
                <p><b>Price:</b> {price:.5f} | <b>RSI:</b> {rsi:.1f}</p>
...
def main():
    ...
    st.title("🚀 Dual Engine Scanner (SMA + BB/KC + Sweeps)")
    
    # NEW: Persistent Warning Message
    st.info("🚨 **IMPORTANT:** Use this scanner in conjunction with the **BTC Market Dashboard (HTML)**. Check BTC Funding and OI before taking signals!", icon="⚠️")
