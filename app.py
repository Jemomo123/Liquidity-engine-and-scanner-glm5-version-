...
# --- Configuration ---
# Failover queue: Binance -> OKX -> Bybit -> Gate.io -> MEXC
EXCHANGE_QUEUE = [
...
def check_sma_sqz(data, len1=20, len2=100):
    ...
    # Logic: Price touches/intersects the area between SMA 20 and SMA 100
...
def main():
    ...
    st.markdown("""
    Scanning for **BB SQZ** and **SMA SQZ** on **3m, 5m, 15m**.
    **System:** Automatically switches exchanges if one is blocked.
    """)
    ...
    if st.button("🚀 Run Scan"):
        ...
        if results:
            ...
            # FIX: Changed applymap to map for Pandas 3.0 compatibility
            st.dataframe(df_results.style.map(color_cells), use_container_width=True)
            st.caption("Green checkmarks indicate an active signal.")
        else:
            st.error("No data found. Check symbol format (e.g., BTC/USDT:USDT).")
