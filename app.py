...
# --- SMA Nearness Engine (User Logic) ---
...
def detect_engines(df):
    ...
    # Duration (Based on SMA Nearness)
    duration = 0
...
def run_scan(pairs, timeframes, btc_regimes, results_placeholder, session_placeholder):
    all_results = []
...
def main():
    ...
    # Logic to handle initial load vs button vs auto-refresh
    # We use a container to hold the loop if auto-refresh is on
    
    # Fetch BTC Regime first
    ...
    if auto_refresh:
        ...
        while True:
            ...
            run_scan(pairs, timeframes, btc_regimes, results_placeholder, session_placeholder)
            elapsed = time.time() - start_time
            sleep_time = max(0, 60 - elapsed)
            time.sleep(sleep_time)
            # Re-fetch regime on loop
            btc_regimes = get_btc_regime()
            with regime_placeholder.container():
                st.markdown("### Multi-TF Bias")
                for tf, data in btc_regimes.items():
        ...
        # Run once on load or button press
        # If scan_btn is False (initial load), we still run it once.
        run_scan(pairs, timeframes, btc_regimes, results_placeholder, session_placeholder)
